...
export default function Developer ()
{
    ...
    return (
        <div className="relative bg-(--container-color) py-8 px-6 rounded-[3rem] grid gap-y-4 overflow-hidden">
                    ...
                    >
                        ...
                        <ul className="flex flex-wrap items-center gap-y-2 gap-x-2">
                            {develops.map((develop) => (
                                    <li key={develop.id} id="service-skill-lists" className="text-[1rem] md:text-[1.25rem] bg-(--container-color) text-(--title-color) py-1 px-2.5 rounded-[4rem] font-unbounded cursor-pointer hover:text-(--first-color) transition duration-400 ease-in-out">{develop.name}</li>
                            ))}
                        </ul>
                    </motion.div>
                )}
