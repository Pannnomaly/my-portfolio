            ...
            <div className="relative z-1">
                ...
                <ul className="flex flex-wrap items-center gap-y-2 gap-x-2">
                    {develops.map((develop) => (
                        <div key={develop.id}>
                            <li id="service-skill-lists" className="text-[1rem] md:text-[1.25rem] bg-(--container-color) text-(--title-color) py-1 px-2.5 rounded-[4rem] font-unbounded">{develop.name}</li>
                        </div>
                    ))}
                </ul>
            </div>
